...
from tools.data_processing.rlcs_processing import get_rlcs_cols, get_rlcs_lines
...
def ingest_data(file_path: str, mode="a", daq_compression=True, daq_aggregate_function="average", msg_packed_filtering="behind_stream", EMPTY_DATA_PLACEHOLDER:Union[int,None] =0) -> Tuple[List[str], List[str], List[str], Union[pd.DataFrame,None], Union[pd.DataFrame,None], Union[pd.DataFrame,None]]:
    ...
    with open(file_path, "rb") as infile:
        ...
        rlcs_cols = get_rlcs_cols(infile)
    ...
    if mode == "a" or mode == "c":
        for col in can_cols:
            ...
            column_counter += 1
    if mode == "a" or mode == "r":
        for col in rlcs_cols:
            column_mapping[("RLCS", col)] = column_counter
            column_counter += 1
    ...
    selection = input(
        "Enter the numbers or a range like 10-12 (or a mix) for the columns you want to extract, seperated by commas, or leave empty for all: ")
    ...
    selected_rlcs_cols = [col[1] for col in column_mapping if col[0]
                         == "RLCS" and column_mapping[col] in indexes]
    ...
    with open(file_path, "rb") as infile:
        ...
        if mode == "a" or mode == "r":
            rlcs_df = get_rlcs_lines(infile, selected_rlcs_cols,placeholder=EMPTY_DATA_PLACEHOLDER)

        else:
            rlcs_df = None

    # offset the timestamps of the data sources so that they start at 0
    offset_timestamps([daq_df, can_df, rlcs_df])

    return selected_daq_cols, selected_can_cols, selected_rlcs_cols, daq_df, can_df, rlcs_df
...
def data_preview(file_path: str, mode="a", msg_packed_filtering="behind_stream"):
    ...
    if mode != "a" and mode != "d" and mode != "c" and mode != "r":
        raise ValueError(f"Invalid mode {mode} passed to data_preview")

    daq_cols, can_cols, rlcs_cols, daq_data, can_data, rlcs_data = ingest_data(file_path, mode, msg_packed_filtering=msg_packed_filtering, EMPTY_DATA_PLACEHOLDER=0)
    ...
    if mode == "a":
        plt.title("DAQ, RLCS, and CAN data")
    elif mode == "d":
        plt.title("DAQ data")
    elif mode == "c":
        plt.title("CAN data")
    ...
    if (mode == "a" or mode == "r") and rlcs_data is not None:
        for i in range(len(rlcs_cols)):
            plt.plot(rlcs_data["timestamp"], rlcs_data[rlcs_cols[i]], label=rlcs_cols[i])
...
def data_export(file_path: str, mode="a", daq_compression=True, daq_aggregate_function="average", msg_packed_filtering="behind_stream"):
    ...
    daq_cols, can_cols, rlcs_cols, daq_data, can_data, rlcs_data = ingest_data(
    ...
    rlcs_data = filter_timestamps(rlcs_data, start, stop)

    # print warnings if the data is empty
    if daq_data is None and can_data is None and rlcs_data is None:
    ...
    export_hash = hashlib.md5(
        f"{file_path}{mode}{start}{stop}{daq_compression}{daq_aggregate_function}{daq_cols}{can_cols}{daq_data}{can_data}{rlcs_cols}{rlcs_data}".encode()).hexdigest()
    export_hash = export_hash[:6]

    formatted_daq_size = "N/A"
    formatted_can_size = "N/A"
    formatted_rlcs_size = "N/A"
    ...
    rlcs_export_path = f"{file_path.replace('.log','')}_export_{export_hash}_rlcs.csv"
    if mode == "a" or mode == "d" and daq_data is not None:
    ...
    if mode == "a" or mode == "r":
        if rlcs_data is not None:
            formatted_rlcs_size = save_data_to_csv(rlcs_export_path, rlcs_data)
            print(f"RLCS data exported to {rlcs_export_path} with size {formatted_rlcs_size}")
        else:
            print("No RLCS data to export")

...
class DAQ_SEND_MESSAGE_TYPE(TypedDict):
    ...
    Unit is seconds.
    """
    # Example: [0.000000019, 0.000000022, 0.000000025] <- 1.3 and 2.3 from above was read at t0 = 0.000000019
...
def read_data(handle, num_addresses, scans_per_read, scan_rate, *, quiet=False, no_built_in_log=False):
    ...
    READ_PERIOD: float = 1 / cast(int, scan_rate)

    rates = []

    # Relative timestamp starting point, starts at current time and scales by READ_PERIOD.
    # Use current time to have a unique starting point on every collection
    relative_last_read_time: float = time.time()

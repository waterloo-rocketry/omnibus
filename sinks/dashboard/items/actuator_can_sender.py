...
from parsley.message_types import actuator_states
...
@Register
class ActuatorCanSender(DashboardItem):
    """
    ...
    This specific style of Sender makes several assumptions about the type of sender and draws input
    from a conifg file containing a description of all of the valve ops states
    """

    def __init__(self, *args):

        super().__init__(*args) # this runs add_parameters

        ACTUATOR_MSG_TEMPLATE = [pf.Enum('actuator', 8, mt.actuator_id)]
        print(ACTUATOR_MSG_TEMPLATE)

        print(list(mt.actuator_id.keys())[0])
        ...
        self.display_can_fields(ACTUATOR_MSG_TEMPLATE)
        
        self.create_send_close_buttton()
        self.create_send_open_buttton()
    ...
    def add_parameters(self):
        actuator_param = SimpleParameter(name='actuator', type='str', default="")
        actuator_param.setValue(list(mt.actuator_id.keys())[0])
        actuator_param.setReadonly(True)
        return [actuator_param]

    def display_can_fields(self, fields: List[pf.Field]):
        self.clear_error_messages()  # remove any preexisiting error lables
        print(fields)
        for field in fields: # When created, this is just one default CAN_MESSAGE field
            self.widget_index += 1
            if isinstance(field, pf.Switch) or isinstance(field, pf.Enum):
                dropdown_items = list(field.get_keys())
                print(field.get_keys())
                ...
                if field.name == 'actuator':
                    dropdown.currentIndexChanged.connect(lambda: 
                        self.parameters.param('actuator').setValue(dropdown.currentText()))

                    self.dropdown = dropdown
                    
                    self.dropdown.setCurrentText(self.parameters.param('actuator').value())
                    self.parameters.param('actuator').sigValueChanged.connect(
                        lambda: self.dropdown.setCurrentText(self.parameters.param('actuator').value()))
                    print('Connect dropbox to parameter')
            ...
            if isinstance(field, pf.Switch):
                ...
                print(field.get_fields(dropdown_items[0]))
    ...
    def update_can_msg(self, text: str):
        ...
        # remove buttons
        self.layout_manager.removeWidget(self.open_button)
        self.open_button.deleteLater()
        self.layout_manager.removeWidget(self.close_button)
    ...
    # whenever the SEND button (or equilvalent) is activated, 
    # pulse PyQT input widgets for visual feedback
    # and upon a successful CAN message encoding, emit the encoded message
    # Due to the purposefully limited nature of the actuator message, many of 
    # the fields are pre-filled.
    def send_can_message(self, actuator_state='ACTUATOR_OFF'):

        can_msg_dict = {
            'msg_type': 'ACTUATOR_CMD',
            'board_id': 'ANY',
            'time': 0.0,
            'actuator': self.parameters.param('actuator').value(),
            'req_state': actuator_state
        }
        ...
        try:
            ...
            can_message = {
                'data': {
                    'time': time.time(),
                    'can_msg': can_msg_dict,  # contains the message data bits
                }
            }
    ...
    def create_send_button(self, type='SEND'):
        self.widget_index += 1
        self.send_button = QPushButton(type)
        ...
        self.send_button.setFixedHeight(
            self.layout_manager.sizeHint().height() - 4*self.layout_manager.verticalSpacing())
        ...
        self.layout_manager.addWidget(self.send_button, 0, self.widget_index, 2, 1)
        self.widgets[self.widget_index - 1] = self.send_button

    def create_send_close_buttton(self):
        self.widget_index += 1
        self.close_button = QPushButton('CLOSE')
        max_text_width = self.get_widget_text_width(self.close_button, max_chars=4)
        self.close_button.setFixedWidth(max_text_width + self.WIDGET_TEXT_PADDING)
        self.close_button.setFixedHeight(
            self.layout_manager.sizeHint().height() - 4*self.layout_manager.verticalSpacing())
        self.close_button.setFocusPolicy(Qt.TabFocus)

        actuator_name = self.parameters.param('actuator').value() 
        if actuator_id_closed_states[actuator_name] == 'ACTUATOR_OFF':
            button_target_state = 'ACTUATOR_OFF'
        else: button_target_state = 'ACTUATOR_ON'

        self.close_button.clicked.connect(lambda: self.send_can_message(button_target_state))
        # add the button to the 2nd row
        self.layout_manager.addWidget(self.close_button, 0, self.widget_index, 2, 1)
        self.widgets[self.widget_index - 1] = self.close_button

    def create_send_open_buttton(self):
        self.widget_index += 1
        self.open_button = QPushButton('OPEN')
        max_text_width = self.get_widget_text_width(self.open_button, max_chars=4)
        self.open_button.setFixedWidth(max_text_width + self.WIDGET_TEXT_PADDING)
        self.open_button.setFixedHeight(
            self.layout_manager.sizeHint().height() - 4*self.layout_manager.verticalSpacing())
        self.open_button.setFocusPolicy(Qt.TabFocus)

        actuator_name = self.parameters.param('actuator').value() 
        if actuator_id_closed_states[actuator_name] == 'ACTUATOR_OFF':
            button_target_state = 'ACTUATOR_ON' 
        else: button_target_state = 'ACTUATOR_OFF' 

        self.open_button.clicked.connect(lambda: self.send_can_message(button_target_state))
        # add the button to the 2nd row
        self.layout_manager.addWidget(self.open_button, 0, self.widget_index, 2, 1)
        self.widgets[self.widget_index - 1] = self.open_button
